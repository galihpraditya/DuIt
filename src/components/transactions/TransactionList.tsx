...
import { Search, Filter, ArrowDownRight, Calendar as CalendarIcon, Tag, RotateCcw, CalendarDays } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-card p-3.5 rounded-3xl space-y-3">
        {/* Search Input (Fixed padding: pl-10 pr-9 to prevent icon overlap) */}
        <div className="flex items-center space-x-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-9 py-2.5 text-xs sm:text-sm rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-slate-800 dark:text-slate-100 placeholder-slate-400"
            />
            {searchQuery && (
              <button
        ...
        <div className="flex items-center gap-2 pt-1 overflow-x-auto whitespace-nowrap scrollbar-none pb-1">
          ...
          <select
            ...
            className="shrink-0 text-xs py-1.5 px-3 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 focus:outline-none"
          >
      ...
      {transactions.length === 0 ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-12 text-center border border-slate-200/80 dark:border-slate-800 shadow-sm">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold shadow-md shadow-emerald-500/20 transition-transform active:scale-95"
          >
        ...
        <div className="space-y-4">
          {groupedTransactions.map((group, groupIdx) => (
            <div key={groupIdx} className="space-y-2">
              ...
              <div className="glass-card rounded-3xl divide-y divide-slate-100 dark:divide-slate-800/80 overflow-hidden">
                {group.items.map((tx) => {
                  const cat = categoryMap.get(tx.categoryId) || {
                    name: 'Lain-lain',
                    icon: 'CircleEllipsis',
                  ...
                  return (
                    <button
                      ...
                      className="w-full text-left p-3.5 sm:p-4 flex items-center justify-between hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors group focus:outline-none focus:bg-slate-50 dark:focus:bg-slate-800/40 active:bg-slate-100 dark:active:bg-slate-800/60"
                    >
                      {/* Left: Icon & Info */}
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                            ...
                            {tx.notes && (
                              <>
                                <span>•</span>
                                <span className="truncate text-slate-600 dark:text-slate-300">{tx.notes}</span>
                              </>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
                        <div className="text-right">
                          <div className="text-sm sm:text-base font-bold text-rose-600 dark:text-rose-400 flex items-center justify-end">
